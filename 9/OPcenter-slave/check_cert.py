...
class Cert_check(object):
    ...
    def py_check(self):
        ...
        #     print(self.domain,e)
            ...
            chacuo_result = self.getChacuoPage()
            if chacuo_result:
                return chacuo_result

            myssl_result = self.getmyssl()
            if myssl_result:
                return myssl_result

            sslchaoshi_result = self.getsslchaoshi()
            if sslchaoshi_result:
                return sslchaoshi_result
            else:
                return self.data

    # 从 https://myssl.com/ 爬取证书信息
    def getmyssl(self):
        print("myssl正在解析：%s" % self.domain)
...
if __name__ == '__main__':
    response = requests.post("http://"+ SERVER +":"+ PORT +"/webmoni/api/domain_all/",data={'node':NODE})
    response_dict = json.loads(response.text)

    for domain_obj in response_dict['data']:

        obj = Cert_check(domain_obj['url'])
        data = obj.py_check()
        print(domain_obj['url'],data)